...
import { Database, Plus, Edit, BookOpen, Clock, Trash2, Upload, HelpCircle, FileText, Check, X } from 'lucide-react';
        ...
        {isFormOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-2xl p-6 sm:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto space-y-6"
            >
              <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">{id ? 'Edit Lesson Unit' : 'Create Lesson Unit'}</h3>
                <button onClick={() => setIsFormOpen(false)} className="p-1.5 hover:bg-slate-105 dark:hover:bg-slate-805 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-white"><X size={18} /></button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-4 bg-slate-50/50 dark:bg-slate-900/40">
                  <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-2">
                    <label className="text-[9px] font-bold text-slate-500 dark:text-slate-450 uppercase tracking-widest block">Formatted Class Study Notes</label>
                    <div className="flex space-x-1">
                      <button
                        type="button"
                        onClick={() => setPreviewNotesTab(false)}
                        className={`px-2.5 py-1 text-[9px] font-bold rounded-lg ${!previewNotesTab ? 'bg-blue-900 text-white' : 'text-slate-650 dark:text-slate-350 hover:bg-slate-200 dark:hover:bg-slate-800'}`}
                      >
                        Editor
                      </button>
                      <button
                        ...
                        className={`px-2.5 py-1 text-[9px] font-bold rounded-lg ${previewNotesTab ? 'bg-blue-900 text-white' : 'text-slate-655 dark:text-slate-355 hover:bg-slate-200 dark:hover:bg-slate-800'}`}
                      >
                        Live Preview
                      </button>
                    </div>
                  ...
                  {!previewNotesTab ? (
                    <div className="space-y-2">
                      {/* Editor Toolbars */}
                      <div className="flex flex-wrap gap-1 bg-white dark:bg-slate-950 p-1 rounded-lg border border-slate-200 dark:border-slate-800">
                        {[
                          ...
                          <button
                            ...
                            type="button"
                            onClick={btn.action}
                            className="px-2 py-1 bg-slate-50 dark:bg-slate-800 hover:bg-slate-150 dark:hover:bg-slate-700 border border-slate-100 dark:border-slate-700 text-[10px] font-bold rounded-md transition-colors text-slate-700 dark:text-slate-300"
                          >
                            {btn.label}
                          </button>
                        ))}
                      ...
                      <textarea
                        ...
                        className="w-full px-3.5 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-805 rounded-xl text-xs font-semibold focus:outline-none focus:border-blue-900 dark:focus:border-yellow-450 h-32 text-slate-805 dark:text-white"
                      />
                    </div>
                  ) : (
                    <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-4 min-h-[140px] text-xs font-sans leading-relaxed text-slate-650 dark:text-slate-400 space-y-4 overflow-y-auto max-h-[180px]">
                      {notes ? (
                        <div dangerouslySetInnerHTML={{ __html: notes }} />
                      ) : (
                        <p className="text-slate-400 italic">No notes text written yet.</p>
                      )}
                    </div>
                  )}
                </div>
